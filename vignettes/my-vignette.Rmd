...
For this example, we will use the `SimData.norm` data file contained in this package. After loading the data, we can see that it contains 2 data matrices saved as a list (`X`) and contains a continuous outcome vector (`Y`). Note that both data matrices have 30 rows or predictors, and have 20 columns or observations. The number of rows need not match across sources, but each source and the outcome must be collected on the same group of individuals.
...
fit
```
Or if we want a more detailed summary, we do the following:
